...
export default async function handler(req, res) {
  ...
  try {
    ...
    const email = userData?.user?.email || req.body?.email;
    if (!userId || !email) return res.status(400).json({ error: "User not found" });
    ...
    await supabaseAdmin
      .from("profiles")
      .update({ basiq_user_id: basiqUserId, has_bank_connection: false })
      .eq("id", userId);
