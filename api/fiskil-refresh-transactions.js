...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ONE fast check only. Frontend will poll.
    ...
    if (!fiskilAccounts.length) {
      console.log("⏳ Fiskil accounts not ready (single-check)", { appUserId, endUserId });
      return res.status(202).json({
        success: false,
        pending: true,
        message:
          "Bank connection is finalising in Fiskil. Accounts are not available yet. Keep retrying.",
