// api/create-consent-session.mjs
...
export default async function handler(req, res) {
  try {
    ...
    if (profileErr) return json(res, 500, { error: "Unable to load profile" });
    ...
    if (!tokenRes.ok || !tokenJson.token) {
      return json(res, 500, { error: "Failed to authenticate with Fiskil", status: tokenRes.status, tokenJson });
    }
    ...
    const endUserRes = await fetch(`${base}/v1/end-users`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${fiskilToken}` },
      body: JSON.stringify({ reference: user.id, email: userEmail }),
    });

    const endUserJson = await safeJson(endUserRes);
    const end_user_id = endUserJson?.id || endUserJson?.end_user_id; // accept both shapes

    if (!endUserRes.ok || !end_user_id) {
      return json(res, 500, { error: "Failed to create Fiskil end user", status: endUserRes.status, endUserJson });
    }
    ...
    const sessionRes = await fetch(`${base}/v1/auth/session`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${fiskilToken}` },
      body: JSON.stringify({ end_user_id, redirect_uri, cancel_uri }),
    });

    const sessionJson = await safeJson(sessionRes);
    if (!sessionRes.ok || !sessionJson?.auth_url) {
      return json(res, 500, { error: "Fiskil auth session failed", status: sessionRes.status, sessionJson });
    }

    return json(res, 200, { auth_url: sessionJson.auth_url, end_user_id });
  } catch (err) {
