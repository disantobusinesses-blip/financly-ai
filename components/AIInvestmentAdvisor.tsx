        ...
        const fetchAdvice = async () => {
            if (isVisible && accounts.length > 0 && user?.membershipType === 'Pro') {
                ...
                try {
                    ...
                    setIsLoading(false);
                    // Set hasFetched here to allow re-fetching when tolerance changes
                    if(!hasFetched) setHasFetched(true); 
                }
            }
        };

        // Only fetch if visible and either it's the first time or risk tolerance has changed
        if (isVisible && (!hasFetched || hasFetched)) {
             fetchAdvice();
        }

    }, [isVisible, accounts, user, riskTolerance, hasFetched]);
